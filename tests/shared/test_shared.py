...
def test_shared_package_is_independent():
    shared_package = Path(".") / "ruth" / "shared"
    ...
    for root, dirs, files in os.walk(shared_package):
        ...
        for file in python_files:
            ...
            rasa_imports = [line for line in imports if "ruth" in line]

            shared_imports = ["ruth.shared", "from ruth import shared"]
            ...
            assert not outside_rasa_imports, (
                ...
                f"of `ruth.shared`: {','.join(outside_rasa_imports)}"
            )
