...
from ruth.shared.exceptions import (
    FileNotFoundException,
    YamlSyntaxException,
    YamlException,
)
import ruth.shared.utils.io
from ruth.shared.constants import LATEST_TRAINING_DATA_FORMAT_VERSION
from ruth.core.actions.action import ACTION_LISTEN_NAME
...
from ruth.shared.core.events import ActionExecuted, UserUttered, SlotSet, ActiveLoop
from ruth.shared.core.training_data.story_reader.yaml_story_reader import (
    YAMLStoryReader,
    DEFAULT_VALUE_TEXT_SLOTS,
)
from ruth.shared.core.training_data.structures import StoryStep, RuleStep
...
def test_is_test_story_file(tmp_path: Path):
    ...
    ruth.shared.utils.io.write_yaml({"stories": []}, path)
...
def test_is_not_test_story_file_if_it_doesnt_contain_stories(tmp_path: Path):
    path = str(tmp_path / "test_stories.yml")
    ruth.shared.utils.io.write_yaml({"nlu": []}, path)
...
- story: my story
  ...
  - user: |
    ...
    story_as_yaml = ruth.shared.utils.io.read_yaml(story)
...
def test_read_mixed_training_data_file(domain: Domain):
    ...
    reader = YAMLStoryReader(domain)
    yaml_content = ruth.shared.utils.io.read_yaml_file(training_data_file)
...
def test_read_from_file_skip_validation(monkeypatch: MonkeyPatch):
    ...
    monkeypatch.setattr(
        sys.modules["ruth.shared.utils.io"],
        ruth.shared.utils.io.read_yaml.__name__,
        Mock(return_value={}),
    )
