...
def test_all_subclasses():
    ...
    assert ruth.shared.utils.common.all_subclasses(TestClass) == expected
...
def test_sort_dicts_by_keys():
    ...
    expected = [{"A": 10}, {"Z": 1}]
    actual = ruth.shared.utils.common.sort_list_of_dicts_by_first_key(test_data)
...
def test_transform_collection_to_sentence(
    ...
    actual = ruth.shared.utils.common.transform_collection_to_sentence(collection)
...
@pytest.mark.parametrize(
    ...
    [
        ("ruth.shared.core.domain.Domain", None, "Domain"),
        # lookup_path
        ("Event", "ruth.shared.core.events", "Event"),
    ],
)
def test_class_from_module_path(
    module_path: Text, lookup_path: Optional[Text], outcome: Text
):
    klass = ruth.shared.utils.common.class_from_module_path(module_path, lookup_path)
...
@pytest.mark.parametrize(
    ...
    [
        ("ruth.shared.core.domain.FunkyDomain", None),
        ("FunkyDomain", None),
        ("FunkyDomain", "ruth.shared.core.domain"),
    ],
)
def test_class_from_module_path_not_found(
    ...
    with pytest.raises(ImportError):
        ruth.shared.utils.common.class_from_module_path(module_path, lookup_path)


@pytest.mark.parametrize(
    "module_path, result, outcome",
    [
        ("ruth.shared.core.domain.Domain", ruth.shared.core.domain.Domain, True),
        ("ruth.shared.core.domain.logger", ruth.shared.core.domain.logger, False),
    ],
)
def test_class_from_module_path_ensure_class(
    module_path: Text, outcome: bool, result: Any, recwarn: WarningsRecorder
):
    klass = ruth.shared.utils.common.class_from_module_path(module_path)
