...
from ruth.shared.exceptions import YamlException, SchemaValidationError
import ruth.shared.utils.io
import ruth.shared.utils.validation as validation_utils
import ruth.utils.io as io_utils
import ruth.shared.nlu.training_data.schemas.data_schema as schema
from ruth.shared.constants import (
    CONFIG_SCHEMA_FILE,
    DOMAIN_SCHEMA_FILE,
    LATEST_TRAINING_DATA_FORMAT_VERSION,
)
from ruth.shared.nlu.training_data.formats.rasa_yaml import NLU_SCHEMA_FILE
...
def test_validate_yaml_schema(file, schema):
    ...
    validation_utils.validate_yaml_schema(ruth.shared.utils.io.read_file(file), schema)
...
def test_validate_yaml_schema_raise_exception(file: Text, schema: Text):
    with pytest.raises(YamlException):
        validation_utils.validate_yaml_schema(
            ruth.shared.utils.io.read_file(file), schema
        )
...
def test_example_training_data_is_valid():
    demo_json = "data/examples/ruth/demo-ruth.json"
    data = ruth.shared.utils.io.read_json_file(demo_json)
...
def test_url_data_format():
    ...
    fname = io_utils.create_temporary_file(
        data.encode(ruth.shared.utils.io.DEFAULT_ENCODING),
        suffix="_tmp_training_data.json",
        mode="w+b",
    )
    data = ruth.shared.utils.io.read_json_file(fname)
...
- intent: goodbye
  examples: |
    ...
    - see you later
        """
        ruth.shared.utils.validation.validate_yaml_schema(payload, NLU_SCHEMA_FILE)
