...
from ruth.shared.constants import CONFIG_AUTOCONFIGURABLE_KEYS
...
DEFAULT_CONFIG = Path("ruth/shared/importers/default_config.yml")


@pytest.mark.parametrize(
    "config_path, autoconfig_keys",
    [
        (Path("ruth/cli/initial_project/config.yml"), {"pipeline", "policies"}),
...
def test_auto_configure(language: Text, keys_to_configure: Set[Text]):
    expected_config = ruth.shared.utils.io.read_config_file(DEFAULT_CONFIG)
...
def test_add_missing_config_keys_to_file(
    ...
    config_after_addition = ruth.shared.utils.io.read_config_file(config_file)
...
def test_dump_config_missing_file(tmp_path: Path, capsys: CaptureFixture):
    ...
    config = ruth.shared.utils.io.read_config_file(str(SOME_CONFIG))
...
def test_dump_config(
    ...
    actual = ruth.shared.utils.io.read_file(config_file)

    expected = ruth.shared.utils.io.read_file(str(CONFIG_FOLDER / expected_file))
...
def test_comment_causing_invalid_autoconfig(tmp_path: Path):
    ...
    dumped = ruth.shared.utils.io.read_yaml_file(config_file)
