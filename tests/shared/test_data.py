...
from ruth.shared.nlu.training_data.loading import load_data
from ruth.shared.utils.io import write_text_file, json_to_string
...
def test_is_yaml_file(path, is_yaml):
    assert ruth.shared.data.is_likely_yaml_file(path) == is_yaml
...
def test_is_md_file(path, is_md):
    assert ruth.shared.data.is_likely_markdown_file(path) == is_md
...
def test_is_json_file(path, is_json):
    assert ruth.shared.data.is_likely_json_file(path) == is_json


def test_story_file_can_not_be_yml(tmpdir: Path):
    p = tmpdir / "test_non_md.yml"
    Path(p).touch()
    assert ruth.shared.data.is_story_file(str()) is False


def test_empty_story_file_is_not_story_file(tmpdir: Path):
    p = tmpdir / "test_non_md.md"
    Path(p).touch()
    assert ruth.shared.data.is_story_file(str(p)) is False
...
def test_default_conversation_tests_are_conversation_tests_yml(tmp_path: Path):
    ...
    assert ruth.shared.data.is_test_stories_file(str(e2e_path))
...
- hallo
    ...
    assert not ruth.shared.data.is_test_stories_file(str(nlu_path))


def test_domain_files_are_not_conversation_tests(tmpdir: Path):
    domain_path = tmpdir / "domain.yml"
    assert not ruth.shared.data.is_test_stories_file(str(domain_path))


async def test_get_files_with_mixed_training_data():
    default_data_path = "data/test_mixed_yaml_training_data/training_data.yml"
    assert ruth.shared.data.get_data_files(
        default_data_path, ruth.shared.data.is_nlu_file
    )
    assert ruth.shared.data.get_data_files(
        default_data_path, ruth.shared.data.is_story_file
    )


def test_get_core_directory(project):
    data_dir = os.path.join(project, "data")
    core_directory = ruth.shared.data.get_core_directory([data_dir])
...
def test_get_nlu_directory(project):
    data_dir = os.path.join(project, "data")
    nlu_directory = ruth.shared.data.get_nlu_directory([data_dir])
...
def test_get_nlu_file(project):
    ...
    nlu_directory = ruth.shared.data.get_nlu_directory(data_file)
...
def test_get_core_nlu_files(project):
    data_dir = os.path.join(project, "data")
    nlu_files = ruth.shared.data.get_data_files(
        [data_dir], ruth.shared.data.is_nlu_file
    )
    core_files = ruth.shared.data.get_data_files(
        [data_dir], ruth.shared.data.is_story_file
    )
    assert len(nlu_files) == 1
...
def test_get_core_nlu_directories(project):
    data_dir = os.path.join(project, "data")
    core_directory, nlu_directory = ruth.shared.data.get_core_nlu_directories(
        [data_dir]
    )
...
def test_get_core_nlu_directories_with_none():
    directories = ruth.shared.data.get_core_nlu_directories(None)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "ruth",
            [
                "data/examples/ruth/demo-ruth-multi-intent.yml",
                "data/examples/ruth/demo-ruth-responses.yml",
                "data/examples/ruth/demo-ruth.json",
                "data/examples/ruth/demo-ruth.yml",
            ],
        ),
...
def test_is_nlu_file_with_json():
    ...
    ruth.shared.utils.io.write_text_file(json_to_string(test), file)

    assert ruth.shared.data.is_nlu_file(file)


def test_is_not_nlu_file_with_json():
    directory = tempfile.mkdtemp()
    file = os.path.join(directory, "test.json")
    ruth.shared.utils.io.write_text_file('{"test": "a"}', file)

    assert not ruth.shared.data.is_nlu_file(file)


def test_get_story_file_with_yaml():
    examples_dir = "data/test_yaml_stories"
    core_files = ruth.shared.data.get_data_files(
        [examples_dir], ruth.shared.data.is_story_file
    )
    assert core_files
