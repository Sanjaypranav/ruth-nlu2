...
from ruth.shared.core.constants import USER_INTENT_OUT_OF_SCOPE
...
from ruth.nlu.convert import convert_training_data
...
from ruth.shared.nlu.training_data.loading import guess_format, UNK, load_data
from ruth.shared.nlu.training_data.util import (
...
from ruth.shared.core.events import UserUttered, ActionExecuted
from ruth.shared.core.training_data.structures import StoryGraph, StoryStep
from ruth.shared.importers.importer import TrainingDataImporter, E2EImporter
...
def test_composite_entities_data():
    td = load_data("data/test/demo-ruth-composite-entities.yml")
    assert not td.is_empty()
...
def test_demo_data(files: List[Text]):
    from ruth.shared.importers.utils import training_data_from_paths
...
@pytest.mark.parametrize(
    "files",
    [
        ("data/examples/ruth/demo-ruth.json", "data/test/multiple_files_json"),
        ("data/examples/ruth/demo-ruth.yml", "data/test/multiple_files_markdown"),
        ("data/examples/ruth/demo-ruth.yml", "data/test/duplicate_intents_yaml"),
    ],
)
    ...
    data = """
{
  "rasa_nlu_data": {
    ...
    f.write_text(data, ruth.shared.utils.io.DEFAULT_ENCODING)
    td = load_data(str(f))
...
@pytest.mark.parametrize(
    ...
    [
        (
            "data/examples/luis/demo-restaurants_v7.json",
            ruth.shared.data.yaml_file_extension(),
        ),
        ("data/examples", ruth.shared.data.yaml_file_extension()),
        (
            "data/test_md/default_retrieval_intents.md",
            ruth.shared.data.markdown_file_extension(),
        ),
        ("data/examples/ruth/demo-ruth.yml", ruth.shared.data.yaml_file_extension()),
        ("data/rasa_yaml_examples", ruth.shared.data.yaml_file_extension()),
    ],
)
