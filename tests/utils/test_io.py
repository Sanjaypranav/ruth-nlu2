...
def test_file_in_path(file, parents):
    assert ruth.shared.utils.io.is_subdirectory(file, parents)
...
def test_file_not_in_path(file, parents):
    assert not ruth.shared.utils.io.is_subdirectory(file, parents)
...
def test_write_utf_8_yaml_file(tmp_path: Path):
    ...
    ruth.shared.utils.io.write_yaml(data, file_path)
    assert ruth.shared.utils.io.read_file(file_path) == "data: amazing 🌈\n"
...
def test_fingerprint_containers(container):
    assert ruth.shared.utils.io.deep_container_fingerprint(
        container
    ) == ruth.shared.utils.io.deep_container_fingerprint(copy.deepcopy(container))


def test_fingerprint_does_not_use_string_hashing(monkeypatch):
    dictionary = {"a": ["b"], "c": {"d": "e"}}
    f1 = ruth.shared.utils.io.deep_container_fingerprint(dictionary)
    ...
    f2 = ruth.shared.utils.io.deep_container_fingerprint(dictionary)
    assert f1 == f2
