...
def test_paired_histogram_specification_bins(
    ...
    for density in [False, True]:
        bins, _, _, _, = ruth.utils.plotting._extract_paired_histogram_specification(
            data,
            num_bins=num_bins,
            density=density,
            x_pad_fraction=0,
...
def test_paired_histogram_specification_bins_raises(bad_data: List):
    ...
    for density in [False, True]:
        with pytest.raises(ValueError):
            ruth.utils.plotting._extract_paired_histogram_specification(
                bad_data,
                num_bins=2,
                density=density,
                x_pad_fraction=0,
...
def test_plot_paired_histogram_warns_on_bad_data(bad_data: List):
    ...
    for density in [False, True]:
        with pytest.warns(
            ...
            ruth.utils.plotting.plot_paired_histogram(
...
def test_paired_histogram_specification_histograms(
    ...
    _, histograms, _, _, = ruth.utils.plotting._extract_paired_histogram_specification(
...
def test_paired_histogram_specification_x_ranges(
    ...
    _, _, x_ranges, _ = ruth.utils.plotting._extract_paired_histogram_specification(
        data,
        num_bins=num_bins,
        density=density,
