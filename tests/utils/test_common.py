...
from ruth.utils.common import RepeatedLogFilter


def test_repeated_log_filter():
    log_filter = RepeatedLogFilter()
    record1 = logging.LogRecord(
        "ruth", logging.INFO, "/some/path.py", 42, "Super msg: %s", ("yes",), None
    )
    record1_same = logging.LogRecord(
        "ruth", logging.INFO, "/some/path.py", 42, "Super msg: %s", ("yes",), None
    )
    record2_other_args = logging.LogRecord(
        "ruth", logging.INFO, "/some/path.py", 42, "Super msg: %s", ("no",), None
    )
    record3_other = logging.LogRecord(
        "ruth", logging.INFO, "/some/path.py", 42, "Other msg", (), None
    )
...
async def test_call_maybe_coroutine_with_async() -> Any:
    ...
    actual = await ruth.utils.common.call_potential_coroutine(my_function())
