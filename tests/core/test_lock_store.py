...
from ruth.core.constants import DEFAULT_LOCK_LIFETIME
from ruth.shared.constants import INTENT_MESSAGE_PREFIX
from ruth.core.lock import TicketLock
import ruth.core.lock_store
from ruth.core.lock_store import (
    InMemoryLockStore,
    LockError,
    LockStore,
    RedisLockStore,
...
def test_raise_connection_exception_redis_lock_store(monkeypatch: MonkeyPatch):
    monkeypatch.setattr(
        ruth.core.lock_store, "RedisLockStore", Mock(side_effect=ConnectionError())
    )
...
async def test_lock_lifetime_environment_variable(monkeypatch: MonkeyPatch):
    ...
    assert ruth.core.lock_store._get_lock_lifetime() == DEFAULT_LOCK_LIFETIME
    ...
    assert ruth.core.lock_store._get_lock_lifetime() == new_lock_lifetime
