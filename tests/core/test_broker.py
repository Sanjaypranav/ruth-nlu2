...
from ruth.core.brokers import pika
from tests.conftest import AsyncMock

import ruth.shared.utils.io
import ruth.utils.io
from ruth.core.brokers.broker import EventBroker
from ruth.core.brokers.file import FileEventBroker
from ruth.core.brokers.kafka import KafkaEventBroker, KafkaProducerInitializationError
from ruth.core.brokers.pika import PikaEventBroker, DEFAULT_QUEUE_NAME
from ruth.core.brokers.sql import SQLEventBroker
from ruth.shared.core.events import Event, Restarted, SlotSet, UserUttered
from ruth.shared.exceptions import ConnectionException, RasaException
...
async def test_file_broker_from_config(tmp_path: Path):
    ...
    ruth.shared.utils.io.write_text_file(endpoint_config, tmp_path / "endpoint.yml")
...
async def test_load_custom_broker_name(tmp_path: Path):
    config = EndpointConfig(
        **{
            "type": "ruth.core.brokers.file.FileEventBroker",
...
async def test_load_non_existent_custom_broker_name():
    config = EndpointConfig(**{"type": "ruth.core.brokers.my.MyProducer"})
...
async def test_no_pika_logs_if_no_debug_mode(caplog: LogCaptureFixture):
    ...
    assert all(
        record.name in ["ruth.core.brokers.pika", "asyncio"]
