...
async def test_agent_with_model_server_in_thread(
    ...
    agent = Agent()
    agent = await ruth.core.agent.load_from_server(
    ...
    agent_policies = {
        ruth.shared.utils.common.module_path_from_instance(p)
...
async def test_wait_time_between_pulls_without_interval(
    ...
    monkeypatch.setattr(
        "ruth.core.agent.schedule_model_pulling", lambda *args: 1 / 0
    ...
    await ruth.core.agent.load_from_server(agent, model_server=model_endpoint_config)
