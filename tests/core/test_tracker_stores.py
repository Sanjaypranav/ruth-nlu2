...
from ruth.shared.constants import DEFAULT_SENDER_ID
...
import ruth.core.tracker_store
from ruth.shared.core.constants import ACTION_LISTEN_NAME, ACTION_SESSION_START_NAME
from ruth.core.constants import POSTGRESQL_SCHEMA
...
from ruth.shared.exceptions import ConnectionException
...
from ruth.utils.endpoints import EndpointConfig, read_endpoint_config
...
def test_exception_tracker_store_from_endpoint_config(
    ...
    monkeypatch.setattr(ruth.core.tracker_store, "RedisTrackerStore", mock)
...
def test_db_get_url_with_sqlite():
    expected = "sqlite:///ruth.db"
    assert str(SQLTrackerStore.get_db_url(dialect="sqlite", db="ruth.db")) == expected
...
def test_session_scope_error(
    ...
    monkeypatch.setattr(
        ruth.core.tracker_store, "ensure_schema_exists", mocked_ensure_schema_exists
    )
...
@pytest.mark.parametrize(
    "url,is_postgres_url",
    [
        (f"{PGDialect.name}://admin:pw@localhost:5432/ruth", True),
...
def test_is_postgres_url(url: Union[Text, URL], is_postgres_url: bool):
    assert ruth.core.tracker_store.is_postgresql_url(url) == is_postgres_url
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            f"{PGDialect.name}://admin:pw@localhost:5432/ruth",
            None,
            {
                "pool_size": ruth.core.tracker_store.POSTGRESQL_DEFAULT_POOL_SIZE,
                "max_overflow": ruth.core.tracker_store.POSTGRESQL_DEFAULT_MAX_OVERFLOW,
            },
        ),
        ...
        (f"{OracleDialect.name}://admin:pw@localhost:5432/ruth", None, {}),
        # oracle with schema
        (f"{OracleDialect.name}://admin:pw@localhost:5432/ruth", "schema1", {}),
        # sqlite
...
def test_create_engine_kwargs(
    ...
    assert ruth.core.tracker_store.create_engine_kwargs(url) == kwargs
...
def test_ensure_schema_exists(
    ...
    set_or_delete_postgresql_schema_env_var(monkeypatch, schema_env)
    monkeypatch.setattr(
        ruth.core.tracker_store, "is_postgresql_url", lambda _: is_postgres
    )
    ...
    with raises_context:
        ruth.core.tracker_store.ensure_schema_exists(session)
...
def test_login_db_with_no_postgresql(tmp_path: Path):
    with pytest.warns(UserWarning):
        SQLTrackerStore(db=str(tmp_path / "ruth.db"), login_db="other")
