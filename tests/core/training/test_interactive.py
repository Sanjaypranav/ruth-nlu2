...
from ruth.core.actions import action
from ruth.core.training import interactive
from ruth.shared.constants import (
    INTENT_MESSAGE_PREFIX,
    DEFAULT_SENDER_ID,
    DOCS_URL_POLICIES,
)
from ruth.shared.core.constants import ACTION_LISTEN_NAME, ACTION_UNLIKELY_INTENT_NAME
from ruth.shared.core.domain import Domain
from ruth.shared.core.events import BotUttered, ActionExecuted, UserUttered
from ruth.shared.core.trackers import DialogueStateTracker
from ruth.shared.core.training_data.story_reader.markdown_story_reader import (
    MarkdownStoryReader,
)
...
from ruth.shared.nlu.constants import TEXT
from ruth.shared.nlu.training_data.formats import RasaYAMLReader, MarkdownReader
from ruth.shared.nlu.training_data.loading import RASA, MARKDOWN, UNK, RASA_YAML
...
def test_latest_user_message():
    ...
    tracker_json = json.loads(ruth.shared.utils.io.read_file(tracker_dump))
...
async def test_print_history(mock_endpoint):
    tracker_dump = ruth.shared.utils.io.read_file(
...
def test_splitting_conversation_at_restarts():
    tracker_dump = "data/test_trackers/tracker_moodbot.json"
    evts = json.loads(ruth.shared.utils.io.read_file(tracker_dump)).get("events")
...
def test_as_md_message():
    parse_data = {
        "text": "Hello there ruth.",
        "entities": [{"start": 12, "end": 16, "entity": "name", "value": "ruth"}],
        "intent": {"name": "greeting", "confidence": 0.9},
    }
    md = interactive._as_md_message(parse_data)
    assert md == "Hello there [ruth](name)."
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "text": "Hello there ruth, it's me, paula.",
                "entities": [
                    {
                        "start": 12,
                        "end": 16,
            ...
            },
            {
                "text": "Hello there ruth, it's me, paula.",
                "entities": [
                    {"start": 12, "end": 16, "entity": "name1", "value": "ruth"},
...
async def test_interactive_domain_persistence(
    ...
    tracker_dump = "data/test_trackers/tracker_moodbot.json"
    tracker_json = ruth.shared.utils.io.read_json_file(tracker_dump)
    ...
    saved_domain = ruth.shared.utils.io.read_config_file(domain_path)
...
def test_no_chat_history_overflow() -> None:
    ...
    ruth.core.training.interactive._chat_history_table(events)
