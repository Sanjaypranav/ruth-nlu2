...
def test_load_config(filename):
    loaded = ruth.core.config.load(filename)
    assert len(loaded) == 2
...
def test_migrate_fallback_policy(
    ...
    updated_config, added_rule = ruth.core.config.migrate_fallback_policies(config)
...
def test_migrate_mapping_policy_to_rules(
    ...
    config, domain, rules = ruth.core.config.migrate_mapping_policy_to_rules(
        config, domain
    )
    ...
    assert (
            ruth.shared.utils.io.read_yaml(rule_writer.dumps(rules)).get("rules", [])
