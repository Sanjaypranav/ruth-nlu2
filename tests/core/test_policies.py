...
from ruth.core.channels import OutputChannel
...
from ruth.shared.nlu.constants import ACTION_NAME, INTENT_NAME_KEY
...
from ruth.shared.core.domain import State, Domain
...
from ruth.core.policies.policy import SupportedData, Policy
...
from ruth.core.policies.memoization import AugmentedMemoizationPolicy, MemoizationPolicy
from ruth.core.policies.sklearn_policy import SklearnPolicy
from ruth.shared.core.trackers import DialogueStateTracker
from ruth.shared.nlu.training_data.formats.markdown import INTENT
...
class TestSklearnPolicy(PolicyTestCollection):
    ...
    def mock_search(self):
        with patch("ruth.core.policies.sklearn_policy.GridSearchCV") as gs:
        ...
        for tr in trackers:
            ...
            for e in tr.applied_events():
                if isinstance(e, ActionExecuted):
                    new_action = ruth.core.actions.action.action_for_index(
...
class TestTwoStageFallbackPolicy(TestFallbackPolicy):
    ...
    async def _get_tracker_after_reverts(
        ...
        tracker = get_tracker(events)
        action = ruth.core.actions.action.ActionRevertFallbackEvents()
