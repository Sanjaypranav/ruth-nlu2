...
import ruth.nlu.components
import ruth.shared.nlu.training_data.loading
from ruth.nlu.components import Component, ComponentBuilder, find_unavailable_packages
from ruth.nlu.config import RasaNLUModelConfig
from ruth.shared.exceptions import InvalidConfigException
from ruth.nlu.model import Interpreter, Metadata, Trainer
...
def test_builder_create_by_module_path(
    ...
    path = "ruth.nlu.featurizers.sparse_featurizer.regex_featurizer.RegexFeaturizer"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "ruth.nlu.featurizers.regex_featurizer.MadeUpClass",
...
def test_warn_of_competing_extractors(
    ...
    if should_warn:
        with pytest.warns(UserWarning):
            ruth.nlu.components.warn_of_competing_extractors(trainer.pipeline)
    else:
...
def test_warn_of_competition_with_regex_extractor(
    ...
    training_data = ruth.shared.nlu.training_data.loading.load_data(data_path)
    ...
    if should_warn:
        with pytest.warns(UserWarning):
            ruth.nlu.components.warn_of_competition_with_regex_extractor(
...
async def test_do_not_warn_for_non_overlapping_entities(tmp_path: Path):
    _, interpreter, _ = await ruth.nlu.train.train(
