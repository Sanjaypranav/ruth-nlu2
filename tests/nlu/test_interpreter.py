...
from ruth.core.interpreter import RasaNLUHttpInterpreter, RasaNLUInterpreter
from ruth.shared.nlu.interpreter import RegexInterpreter
from ruth.model import get_model_subdirectories, get_model
...
def test_model_is_not_compatible(metadata):
    with pytest.raises(ruth.nlu.model.UnsupportedModelError):
        Interpreter.ensure_model_compatibility(metadata)


@pytest.mark.parametrize("metadata", [{"rasa_version": ruth.__version__}])
...
def test_create_interpreter(parameters, trained_nlu_model):
    ...
    interpreter = ruth.core.interpreter.create_interpreter(
