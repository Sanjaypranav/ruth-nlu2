...
import ruth.shared.nlu.training_data.message
...
def test_list_files_ignores_hidden_files(tmpdir):
    ...
    assert ruth.shared.utils.io.list_files(tmpdir.strpath) == [normal_file]


def test_creation_of_existing_dir(tmpdir):
    # makes sure there is no exception
    assert ruth.shared.utils.io.create_directory(tmpdir.strpath) is None
