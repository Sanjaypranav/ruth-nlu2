...
async def _train_persist_load_with_different_settings(
    ...
    (trainer, trained, persisted_path) = await ruth.nlu.train.train(
        _config,
        path=str(tmp_path),
        data="data/examples/ruth/demo-ruth-multi-intent.yml",
...
async def test_set_random_seed(component_builder, tmpdir, nlu_as_json_path: Text):
    ...
    # first run
    (trained_a, _, persisted_path_a) = await ruth.nlu.train.train(
        _config,
    ...
    # second run
    (trained_b, _, persisted_path_b) = await ruth.nlu.train.train(
        _config,
...
async def test_train_persist_load_with_composite_entities(
    ...
    (trainer, trained, persisted_path) = await ruth.nlu.train.train(
        _config,
        path=tmpdir.strpath,
        data="data/test/demo-ruth-composite-entities.yml",
        component_builder=component_builder,
    )
