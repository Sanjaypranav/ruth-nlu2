...
from ruth.nlu.train import Trainer, Interpreter
...
from ruth.utils.tensorflow.model_data_utils import FeatureArray
...
def test_train_selector(pipeline, component_builder, tmpdir):
    ...
    training_data = ruth.shared.nlu.training_data.loading.load_data(
        "data/examples/ruth/demo-ruth.yml"
    )
    training_data_responses = ruth.shared.nlu.training_data.loading.load_data(
...
async def _train_persist_load_with_different_settings(
    ...
    (trainer, trained, persisted_path) = await ruth.nlu.train.train(
        _config,
        path=str(tmp_path),
        data="data/examples/ruth/demo-ruth.yml",
        component_builder=component_builder,
    )
...
async def test_cross_entropy_with_linear_norm(
    ...
    (trained_model, _, persisted_path) = await ruth.nlu.train.train(
        _config,
        path=str(tmp_path),
        data="data/test_selectors",
        component_builder=component_builder,
...
async def test_adjusting_layers_incremental_training(
    ...
    (_, trained, persisted_path) = await ruth.nlu.train.train(
        _config,
        path=str(tmpdir),
    ...
    assert loaded.pipeline
    assert loaded.parse("Rasa is great!") == trained.parse("Rasa is great!")
    (_, trained, _) = await ruth.nlu.train.train(
        _config,
        path=str(tmpdir),
