...
from ruth.core.policies.ted_policy import TEDPolicy
import ruth.model
import ruth.core
import ruth.core.train
import ruth.nlu
from ruth.nlu.classifiers.diet_classifier import DIETClassifier
import ruth.shared.importers.autoconfig as autoconfig
import ruth.shared.utils.io
from ruth.core.agent import Agent
from ruth.core.interpreter import RasaNLUInterpreter
from ruth.nlu.model import Interpreter

from ruth.model_training import train_core, train_nlu, train, dry_run_result
from ruth.utils.tensorflow.constants import EPOCHS
...
def test_package_model(trained_rasa_model: Text, parameters: Dict):
    ...
    train_path = ruth.model.unpack_model(trained_rasa_model)

    model_path = ruth.model.package_model(
        _fingerprint(),
        output_path,
        train_path,
...
def test_interpreter_of_old_model_passed_to_core_training(
    ...
    # NLU isn't retrained
    monkeypatch.setattr(
        ruth.model.FingerprintComparisonResult,
        ruth.model.FingerprintComparisonResult.should_retrain_nlu.__name__,
        lambda _: False,
    )

    # An old model with an interpreter exists
    monkeypatch.setattr(
        ruth.model, ruth.model.get_latest_model.__name__, lambda _: trained_rasa_model
    )
...
def test_load_interpreter_returns_none_for_none():
    from ruth.model_training import _load_interpreter

    assert _load_interpreter(None) is None


def test_interpreter_from_previous_model_returns_none_for_none():
    from ruth.model_training import _interpreter_from_previous_model
...
def test_train_core_autoconfig(
    ...
    monkeypatch.setattr(
        ruth.model_training, "_train_core_with_validated_data", AsyncMock()
    )
...
def test_train_nlu_autoconfig(
    ...
    monkeypatch.setattr(
        ruth.model_training, "_train_nlu_with_validated_data", AsyncMock()
    )
...
def mock_core_training(monkeypatch: MonkeyPatch) -> Mock:
    return mock_async(monkeypatch, ruth.core.train, ruth.core.train.train.__name__)


def mock_nlu_training(monkeypatch: MonkeyPatch) -> Mock:
    return mock_async(monkeypatch, ruth.nlu.train, ruth.nlu.train.train.__name__)
...
class TestE2e:
    ...
    def test_retrains_nlu_and_core_if_new_e2e_example(
        ...
        stories_yaml = ruth.shared.utils.io.read_yaml_file(e2e_stories_path)
        stories_yaml["stories"][1]["steps"].append({"user": "new message!"})

        new_stories_file = tmp_path / "new_stories.yml"
        ruth.shared.utils.io.write_yaml(stories_yaml, new_stories_file)
    ...
    def test_new_nlu_data_retrains_core_if_there_are_e2e_stories(
        ...
        nlu_yaml = ruth.shared.utils.io.read_yaml_file(nlu_data_path)
        nlu_yaml["nlu"][0]["examples"] += "- surprise!\n"

        new_nlu_file = tmp_path / "new_nlu.yml"
        ruth.shared.utils.io.write_yaml(nlu_yaml, new_nlu_file)
    ...
    def test_training_core_with_e2e_fails_gracefully(
        ...
        assert (
            ...
            "Please train using `ruth train` so that the NLU model is also trained."
        ) in captured.out
...
def test_model_finetuning_core(
    ...
    old_config = ruth.shared.utils.io.read_yaml_file("data/test_moodbot/config.yml")
    old_config["policies"][0]["epochs"] = 10
    new_config_path = tmp_path / "new_config.yml"
    ruth.shared.utils.io.write_yaml(old_config, new_config_path)

    old_stories = ruth.shared.utils.io.read_yaml_file(
    ...
    ruth.shared.utils.io.write_yaml(old_stories, new_stories_path)
...
def test_model_finetuning_core_new_domain_label(
    ...
    old_domain = ruth.shared.utils.io.read_yaml_file("data/test_moodbot/domain.yml")
    old_domain["intents"].append("a_new_one")
    new_domain_path = tmp_path / "new_domain.yml"
    ruth.shared.utils.io.write_yaml(old_domain, new_domain_path)
...
def test_model_finetuning_nlu(
    ...
    old_nlu = ruth.shared.utils.io.read_yaml_file("data/test_moodbot/data/nlu.yml")
    old_nlu["nlu"][-1]["examples"] = "-something else"
    new_nlu_path = tmp_path / "new_nlu.yml"
    ruth.shared.utils.io.write_yaml(old_nlu, new_nlu_path)
...
def test_model_finetuning_nlu_new_label_already_in_domain(
    ...
    old_nlu = ruth.shared.utils.io.read_yaml_file(nlu_data_path)
...
def test_dry_run_result(
    result: ruth.model.FingerprintComparisonResult, code: int, texts_count: int,
):
