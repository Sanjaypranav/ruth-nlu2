...
from ruth.shared.core.domain import KEY_RESPONSES
...
from ruth.model_training import train_core_async
...
def test_get_local_model(trained_rasa_model: str):
    assert ruth.model.get_local_model(trained_rasa_model) == trained_rasa_model


@pytest.mark.parametrize("model_path", ["foobar", "ruth", "README.md", None])
def test_get_local_model_exception(model_path: Optional[Text]):
    with pytest.raises(ModelNotFound):
        ruth.model.get_local_model(model_path)
...
def test_persist_and_load_fingerprint():
    from ruth.model import persist_fingerprint, fingerprint_from_path
...
async def test_fingerprinting_changing_config_epochs(project: Text, tmp_path):
    ...
    ruth.shared.utils.io.write_yaml(config1, config1_path, True)
    ...
    ruth.shared.utils.io.write_yaml(config2, config2_path, True)
    ...
    ruth.shared.utils.io.write_yaml(config3, config3_path, True)
...
async def test_can_finetune_min_version(
    ...
    monkeypatch.setattr(
        ruth.constants, "MINIMUM_COMPATIBLE_VERSION", min_compatible_version
    )
    monkeypatch.setattr(ruth, "__version__", old_model_version)
    old_fingerprint = await model_fingerprint(importer)
    new_fingerprint = await model_fingerprint(importer)

    with mock.patch("ruth.model.MINIMUM_COMPATIBLE_VERSION", min_compatible_version):
