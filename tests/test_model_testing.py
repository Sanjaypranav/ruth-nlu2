...
def test_get_sanitized_model_directory_when_passing_model_file_explicitly(
    ...
    assert str(other_model) != ruth.model.get_latest_model()
    other_model.touch()
...
def test_get_label_set(targets, exclude_label, expected):
    from ruth.model_testing import get_unique_labels
...
def test_e2e_warning_if_no_nlu_model(
    ...
    from ruth.model_testing import test_core

    # Patching is bit more complicated as we have a module `train` and function
    # with the same name 😬
    monkeypatch.setattr(
        sys.modules["ruth.core.test"], "test", asyncio.coroutine(lambda *_, **__: True)
    )
...
def test_log_failed_stories(tmp_path: Path):
    ...
    ruth.core.test._log_stories([], path, "Some text")

    dump = ruth.shared.utils.io.read_file(path)
...
async def test_test_does_not_use_rules(tmp_path: Path, default_agent: Agent):
    from ruth.core.test import _create_data_generator
