...
from ruth.cli import data
from ruth.core.constants import (
    DEFAULT_NLU_FALLBACK_AMBIGUITY_THRESHOLD,
    DEFAULT_NLU_FALLBACK_THRESHOLD,
    DEFAULT_CORE_FALLBACK_THRESHOLD,
)
from ruth.shared.constants import LATEST_TRAINING_DATA_FORMAT_VERSION, DEFAULT_DATA_PATH
...
def test_data_split_nlu(run_in_simple_project: Callable[..., RunResult]):
    ...
    for yml_file in nlu_files:
        ...
        nlu_data = ruth.shared.utils.io.read_yaml_file(yml_file)
...
def test_data_split_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth data split nlu [-h] [-v] [-vv] [--quiet] [-u NLU]
...
def test_data_convert_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth data convert nlu [-h] [-v] [-vv] [--quiet] [-f {json,md,yaml}]
...
def test_data_validate_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth data validate [-h] [-v] [-vv] [--quiet]
...
def test_data_validate_stories_with_max_history_zero(monkeypatch: MonkeyPatch):
    ...
    monkeypatch.setattr("ruth.validator.Validator.from_importer", mock_from_importer)
...
def test_convert_config(
    ...
    ruth.shared.utils.io.write_yaml(deprecated_config, config_file)
    ...
    assert result.ret == 0
    new_config = ruth.shared.utils.io.read_yaml_file(config_file)
    new_domain = ruth.shared.utils.io.read_yaml_file(domain_file)
    new_rules = ruth.shared.utils.io.read_yaml_file(rules_file)
...
def test_convert_config_with_output_file_containing_data(
    ...
    existing_rules = ruth.shared.utils.io.read_yaml_file(output_file)["rules"]
    ...
    new_rules = ruth.shared.utils.io.read_yaml_file(output_file)["rules"]
    expected_new_rule = {
        "rule": "Rule to handle messages with low NLU confidence "
        "(automated conversion from 'FallbackPolicy')",
