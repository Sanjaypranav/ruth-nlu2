...
from ruth.shared.nlu.training_data import training_data
from ruth.model_training import (
    CODE_CORE_NEEDS_TO_BE_RETRAINED,
    CODE_FORCED_TRAINING,
)

# noinspection PyProtectedMember
from ruth.cli.train import _get_valid_config
...
def test_train(run_in_simple_project: Callable[..., RunResult]):
    ...
    files = ruth.shared.utils.io.list_files(os.path.join(temp_dir, "train_models"))
    assert len(files) == 1
...
def test_train_no_domain_exists(
    ...
    files = ruth.shared.utils.io.list_files("train_models_no_domain")
    assert len(files) == 1
...
def test_train_skip_on_model_not_changed(
    ...
    files = ruth.shared.utils.io.list_files(os.path.join(temp_dir, "models"))
    assert len(files) == 1
...
def test_train_core(run_in_simple_project: Callable[..., RunResult]):
    ...
    assert os.path.exists("train_rasa_models/ruth-model.tar.gz")
    assert os.path.isfile("train_rasa_models/ruth-model.tar.gz")
...
def test_train_core_no_domain_exists(run_in_simple_project: Callable[..., RunResult]):
    ...
    assert not os.path.exists("train_rasa_models_no_domain/ruth-model.tar.gz")
    assert not os.path.isfile("train_rasa_models_no_domain/ruth-model.tar.gz")
...
def test_train_nlu(run_in_simple_project: Callable[..., RunResult]):
    ...
    files = ruth.shared.utils.io.list_files("train_models")
    assert len(files) == 1
...
def test_train_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth train [-h] [-v] [-vv] [--quiet] [--data DATA [DATA ...]]
...
def test_train_nlu_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth train nlu [-h] [-v] [-vv] [--quiet] [-c CONFIG] [-d DOMAIN]
...
def test_train_core_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth train core [-h] [-v] [-vv] [--quiet] [-s STORIES] [-d DOMAIN]
...
def test_get_valid_config(parameters):
    ...
    if parameters["config_data"] is not None:
        ...
        ruth.shared.utils.io.write_yaml(parameters["config_data"], config_path)
    ...
    if parameters["error"]:
        ...
        config_data = ruth.shared.utils.io.read_yaml_file(config_path)
