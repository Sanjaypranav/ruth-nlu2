...
def test_x_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth x [-h] [-v] [-vv] [--quiet] [-m MODEL] [--data DATA [DATA ...]]
              [-c CONFIG] [-d DOMAIN] [--no-prompt] [--production]
              [--ruth-x-port RASA_X_PORT] [--config-endpoint CONFIG_ENDPOINT]
...
def test_prepare_credentials_for_rasa_x_if_rasa_channel_not_given(tmpdir: Path):
    ...
    ruth.shared.utils.io.write_yaml({}, credentials_path)

    tmp_credentials = x._prepare_credentials_for_rasa_x(
        credentials_path, "http://localhost:5002"
    )

    actual = ruth.shared.utils.io.read_config_file(tmp_credentials)

    assert actual["ruth"]["url"] == "http://localhost:5002"
...
def test_prepare_credentials_if_already_valid(tmpdir: Path):
    ...
    credentials = {
        "ruth": {"url": "my-custom-url"},
        "another-channel": {"url": "some-url"},
    }
    ruth.shared.utils.io.write_yaml(credentials, credentials_path)

    x._prepare_credentials_for_rasa_x(credentials_path)

    actual = ruth.shared.utils.io.read_config_file(credentials_path)
...
async def test_pull_runtime_config_from_server():
    ...
    credentials = "ruth: http://example.com:5002/api"
    endpoint_config = """
    event_broker:
    ...
    with aioresponses() as mocked:
        ...
        assert ruth.shared.utils.io.read_file(endpoints_path) == endpoint_config
        assert ruth.shared.utils.io.read_file(credentials_path) == credentials
