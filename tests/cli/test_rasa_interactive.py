...
from ruth.core.training import interactive as interactive_learning
from ruth.cli import interactive, train
...
def test_interactive_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth interactive [-h] [-v] [-vv] [--quiet] [--e2e] [-p PORT] [-m MODEL]
...
def test_interactive_core_help(run: Callable[..., RunResult]):
    ...
    help_text = """usage: ruth interactive core [-h] [-v] [-vv] [--quiet] [-m MODEL] [-s STORIES]
                             [--skip-visualization]
                             [--conversation-id CONVERSATION_ID]
                             [--endpoints ENDPOINTS] [-c CONFIG] [-d DOMAIN]
                             [--out OUT] [--augmentation AUGMENTATION]
...
def test_pass_arguments_to_rasa_train(
    ...
    monkeypatch.setattr(ruth, "train", mock.method)
