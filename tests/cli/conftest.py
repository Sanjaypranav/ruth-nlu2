...
from ruth.cli import scaffold
from ruth.shared.utils.io import write_yaml


@pytest.fixture
def run(testdir: Testdir) -> Callable[..., RunResult]:
    def do_run(*args):
        args = [shutil.which("ruth")] + list(args)
...
def trained_simple_project(tmpdir_factory: TempdirFactory) -> Text:
    ...
    check_call([shutil.which("ruth"), "train"], cwd=path.strpath)
