...
import ruth
import ruth.constants
import ruth.core.jobs
import ruth.nlu
import ruth.server
import ruth.shared.constants
import ruth.shared.utils.io
import ruth.utils.io
from ruth.core import utils
from ruth.core.agent import Agent, load_agent
from ruth.core.channels import (
    channel,
...
from ruth.shared.core.domain import Domain, SessionConfig
...
from ruth.model_training import TrainingResult
...
async def tear_down_scheduler() -> Generator[None, None, None]:
    yield None
    ruth.core.jobs.__scheduler = None
...
async def test_version(rasa_non_trained_app: SanicASGITestClient):
    ...
    assert response.status == HTTPStatus.OK
    assert content.get("version") == ruth.__version__
    assert (
            content.get("minimum_compatible_version")
            == ruth.constants.MINIMUM_COMPATIBLE_VERSION
    )
...
def background_server(
    ...
    def run_server(monkeypatch: MonkeyPatch) -> NoReturn:
        ...
        monkeypatch.setattr(
            sys.modules["ruth.model_training"], "train_async", mocked_training_function,
        )

        from ruth import __main__

        sys.argv = ["ruth", "run", "--enable-api"]
        __main__.main()
...
def training_request(
    ...
    def send_request() -> None:
        ...
        for file in [
            ...
            content = ruth.shared.utils.io.read_yaml_file(full_path)
            payload.update(content)

        concatenated_payload_file = tmp_path / "concatenated.yml"
        ruth.shared.utils.io.write_yaml(payload, concatenated_payload_file)
        ...
        response = requests.post(
            ...
            headers={"Content-type": ruth.server.YAML_CONTENT_TYPE},
...
async def test_train_nlu_success(
    ...
    domain_data = ruth.shared.utils.io.read_yaml_file(domain_path)
    config_data = ruth.shared.utils.io.read_yaml_file(stack_config_path)
    nlu_data = ruth.shared.utils.io.read_yaml_file(nlu_data_path)
    ...
    data = StringIO()
    ruth.shared.utils.io.write_yaml(payload, data)
...
def test_deprecation_warnings_json_payload(payload: Dict):
    with pytest.warns(FutureWarning):
        ruth.server._validate_json_training_payload(payload)
...
async def test_train_with_yaml_with_params(
    ...
    monkeypatch.setattr(ruth.model_training, "train_async", mock_train)
...
def test_training_payload_from_yaml_force_training(
    ...
    payload = ruth.server._training_payload_from_yaml(request, tmp_path)
...
@pytest.mark.parametrize(
    "headers, expected",
    [
        ({}, ruth.shared.constants.DEFAULT_MODELS_PATH),
...
async def test_evaluate_stories(rasa_app: SanicASGITestClient, stories_path: Text):
    stories = ruth.shared.utils.io.read_file(stories_path)
...
async def test_evaluate_stories_end_to_end_md(
    ...
    stories = ruth.shared.utils.io.read_file(end_to_end_story_md_path)
...
async def test_evaluate_stories_end_to_end(
    ...
    stories = ruth.shared.utils.io.read_file(end_to_end_story_path)
...
async def test_evaluate_intent(rasa_app: SanicASGITestClient, nlu_data_path: Text):
    nlu_data = ruth.shared.utils.io.read_file(nlu_data_path)
...
async def test_evaluate_intent_json(rasa_app: SanicASGITestClient):
    nlu_data = ruth.shared.utils.io.read_file("data/test/demo-ruth-small.json")

    _, response = await rasa_app.post(
        "/model/test/intents",
        json=nlu_data,
        headers={"Content-type": ruth.server.JSON_CONTENT_TYPE},
    )
...
async def test_evaluate_intent_without_body(rasa_app: SanicASGITestClient):
    _, response = await rasa_app.post(
        "/model/test/intents", headers={"Content-type": ruth.server.YAML_CONTENT_TYPE},
    )
...
async def test_cross_validation_with_callback_success(
    ...
    with aioresponses() as mocked:
        ...
        monkeypatch.setattr(
            ruth.nlu.test,
            ruth.nlu.test.cross_validate.__name__,
...
async def test_callback_unexpected_error(
    ...
    monkeypatch.setattr(
        ruth.server,
        ruth.server._training_payload_from_yaml.__name__,
        Mock(side_effect=ValueError()),
    )
...
async def test_predict_empty_request_body(rasa_app: SanicASGITestClient):
    _, response = await rasa_app.post(
        "/model/predict", headers={"Content-Type": ruth.server.JSON_CONTENT_TYPE},
    )
    assert response.status == HTTPStatus.BAD_REQUEST
...
def test_list_routes(empty_agent: Agent):
    app = ruth.server.create_app(empty_agent, auth_token=None)
...
async def test_get_domain(rasa_app: SanicASGITestClient):
    _, response = await rasa_app.get(
        "/domain", headers={"accept": ruth.server.JSON_CONTENT_TYPE}
    )
...
def test_get_output_channel(
    ...
    actual = ruth.server._get_output_channel(request, None)
...
def test_get_latest_output_channel(input_channels: List[Text], expected_channel: Type):
    ...
    actual = ruth.server._get_output_channel(request, tracker)
...
def test_app_when_app_has_no_input_channels():
    ...
    actual = ruth.server._get_output_channel(
...
async def test_update_conversation_with_events(
    ...
    fetched_tracker = await ruth.server.update_conversation_with_events(
