...
from ruth.shared.utils.common import lazy_property
...
from ruth.shared.nlu.training_data import util
...
class TrainingData:
    ...
    def _load_lookup_table(lookup_table: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if Path(potential_file).is_file():
            try:
                lookup_table["elements"] = ruth.shared.utils.io.read_file(
                    potential_file
                )
                return lookup_table
    ...
    def fingerprint(self) -> Text:
        ...
        return ruth.shared.utils.io.deep_container_fingerprint(relevant_attributes)
    ...
    def label_fingerprint(self) -> Text:
        ...
        return ruth.shared.utils.io.deep_container_fingerprint(labels)
    ...
    def nlu_as_json(self, **kwargs: Any) -> Text:
        ...
        from ruth.shared.nlu.training_data.formats import RasaWriter
    ...
    def nlg_as_markdown(self) -> Text:
        ...
        from ruth.shared.nlu.training_data.formats import NLGMarkdownWriter
    ...
    def nlg_as_yaml(self) -> Text:
        ...
        """
        from ruth.shared.nlu.training_data.formats.rasa_yaml import RasaYAMLWriter
    ...
    def nlu_as_markdown(self) -> Text:
        ...
        from ruth.shared.nlu.training_data.formats import MarkdownWriter
    ...
    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH) -> None:
        if ruth.shared.data.is_likely_json_file(filename):
            ruth.shared.utils.io.write_text_file(self.nlu_as_json(indent=2), filename)
        elif ruth.shared.data.is_likely_markdown_file(filename):
            ruth.shared.utils.io.write_text_file(self.nlu_as_markdown(), filename)
        elif ruth.shared.data.is_likely_yaml_file(filename):
            ruth.shared.utils.io.write_text_file(self.nlu_as_yaml(), filename)
        else:
            raise ValueError(
    ...
    def persist_nlg(self, filename: Text) -> None:
        if ruth.shared.data.is_likely_yaml_file(filename):
            ruth.shared.utils.io.write_text_file(self.nlg_as_yaml(), filename)
        elif ruth.shared.data.is_likely_markdown_file(filename):
            nlg_serialized_data = self.nlg_as_markdown()
            if nlg_serialized_data:
                ruth.shared.utils.io.write_text_file(nlg_serialized_data, filename)
        else:
            raise ValueError(
    ...
    def get_nlg_persist_filename(nlu_filename: Text) -> Text:
        ...
        if ruth.shared.data.is_likely_json_file(nlu_filename):
            ...
            extension = ruth.shared.data.markdown_file_extension()
