...
def get_file_format_extension(resource_name: Text) -> Text:
    ...
    if not file_formats:
        return ruth.shared.data.yaml_file_extension()

    known_file_formats = {
        loading.MARKDOWN: ruth.shared.data.markdown_file_extension(),
        loading.RASA_YAML: ruth.shared.data.yaml_file_extension(),
    }
    fformat = file_formats[0]
    if all(f == fformat for f in file_formats):
        return known_file_formats.get(fformat, ruth.shared.data.yaml_file_extension())
...
def remove_untrainable_entities_from(example: Dict[Text, Any]) -> None:
    ...
    are defined in `ruth.nlu.constants.PRETRAINED_EXTRACTORS`.
