...
from ruth.shared.nlu.training_data.formats.readerwriter import TrainingDataReader
from ruth.shared.nlu.training_data.util import transform_entity_synonyms
...
DOCS_URL_MIGRATE_GOOGLE = DOCS_BASE_URL + "/migrate-from/google-dialogflow-to-ruth/"
...
class DialogflowReader(TrainingDataReader):
    def read(self, fn: Text, **kwargs: Any) -> "TrainingData":
        ...
        root_js = ruth.shared.utils.io.read_json_file(fn)
    ...
    def _extract_entity(
        ...
        if "meta" in chunk or "alias" in chunk:
            ...
            if entity_type != "@sys.ignore":
                entity = ruth.shared.nlu.training_data.util.build_entity(
    ...
    def _read_examples(
        ...
        if os.path.isfile(examples_fn):
            return ruth.shared.utils.io.read_json_file(examples_fn)
        else:
            return None
