...
from ruth.shared.utils import validation
...
from ruth.shared.nlu.constants import METADATA_INTENT, METADATA_EXAMPLE
from ruth.shared.nlu.training_data.formats.readerwriter import (
    TrainingDataReader,
    TrainingDataWriter,
)
...
class RasaYAMLReader(TrainingDataReader):
    ...
    def _parse_intent(self, intent_data: Dict[Text, Any]) -> None:
        import ruth.shared.nlu.training_data.entities_parser as entities_parser
        import ruth.shared.nlu.training_data.synonyms_parser as synonyms_parser
    ...
    def _parse_lookup(self, nlu_item: Dict[Text, Any]) -> None:
        import ruth.shared.nlu.training_data.lookup_tables_parser as lookup_tables_parser  # noqa: E501
    ...
    def is_yaml_nlu_file(filename: Union[Text, Path]) -> bool:
        ...
        """
        if not ruth.shared.data.is_likely_yaml_file(filename):
            return False

        return ruth.shared.utils.io.is_key_in_yaml(filename, KEY_NLU, KEY_RESPONSES)
...
class RasaYAMLWriter(TrainingDataWriter):
    ...
    def dump(
        ...
        if result:
            ruth.shared.utils.io.write_yaml(result, target, True)
    ...
    def process_training_examples_by_key(
        ...
        for intent_name, examples in training_examples.items():
            ...
            example_texts_have_escape_chars = any(
                ruth.shared.nlu.training_data.util.has_string_escape_chars(
