...
from ruth.shared.nlu.training_data.formats import MarkdownReader, NLGMarkdownReader
from ruth.shared.nlu.training_data.formats.dialogflow import (
    DIALOGFLOW_AGENT,
    DIALOGFLOW_ENTITIES,
...
def load_data(resource_name: Text, language: Optional[Text] = "en") -> "TrainingData":
    ...
    if os.path.isfile(resource_name):
        ...
        files = ruth.shared.utils.io.list_files(resource_name)
...
def _reader_factory(fformat: Text) -> Optional["TrainingDataReader"]:
    ...
    from ruth.shared.nlu.training_data.formats import (
        RasaYAMLReader,
        MarkdownReader,
        WitReader,
        LuisReader,
