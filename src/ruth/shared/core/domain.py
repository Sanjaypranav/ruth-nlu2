...
import ruth.shared.core.constants
from ruth.shared.exceptions import RasaException, YamlException, YamlSyntaxException
...
from ruth.shared.core.events import SlotSet, UserUttered
from ruth.shared.core.slots import Slot, CategoricalSlot, TextSlot, AnySlot
from ruth.shared.utils.validation import KEY_TRAINING_DATA_FORMAT_VERSION
from ruth.shared.constants import RESPONSE_CONDITION
    ...
    for response_variations in responses.values():
        for variation in response_variations:
            if RESPONSE_CONDITION in variation:
                ruth.shared.utils.common.mark_as_experimental_feature(
...
class Domain:
    ...
    def from_file(cls, path: Text) -> "Domain":
        ...
        return cls.from_yaml(ruth.shared.utils.io.read_file(path), path)

    @classmethod
    def from_yaml(cls, yaml: Text, original_filename: Text = "") -> "Domain":
        """Loads the `Domain` from YAML text after validating it."""
        try:
            ruth.shared.utils.validation.validate_yaml_schema(yaml, DOMAIN_SCHEMA_FILE)

            data = ruth.shared.utils.io.read_yaml(yaml)
            if not ruth.shared.utils.validation.validate_training_data_format_version(
    ...
    def _transform_intent_properties_for_internal_use(
        ...
        if properties:
            ...
            raise InvalidDomain(
                ...
                f" {ruth.shared.constants.DOCS_URL_DOMAINS} for more information on how"
                f" to correctly add `intents` in the `domain` and"
                f" {ruth.shared.constants.DOCS_URL_INTENTS} for examples on"
    ...
    @ruth.shared.utils.common.lazy_property
                ...
                for _entity, sub_labels in entity.items():
                    ...
                    if sub_labels:
                        ...
                        raise InvalidDomain(
                            ...
                            f" {ruth.shared.constants.DOCS_URL_DOMAINS} for more"
                            f" information on how to correctly add `entities` in the"
                            f" `domain` and {ruth.shared.constants.DOCS_URL_ENTITIES}"
    ...
    def _add_default_intents(
        ...
        for intent_name in ruth.shared.core.constants.DEFAULT_INTENTS:
    ...
    def _collect_overridden_default_intents(
        ...
        return sorted(
            intent_names.intersection(set(ruth.shared.core.constants.DEFAULT_INTENTS))
        )
    ...
    def _initialize_forms(
        ...
        if isinstance(forms, list) and (not forms or isinstance(forms[0], str)):
            ...
            ruth.shared.utils.io.raise_warning(
                ...
                "Source 3.0.",
                docs=ruth.shared.constants.DOCS_URL_FORMS,
    ...
    def fingerprint(self) -> Text:
        ...
        self_as_dict[
            KEY_INTENTS
        ] = ruth.shared.utils.common.sort_list_of_dicts_by_first_key(
            self_as_dict[KEY_INTENTS]
        )
        self_as_dict[KEY_ACTIONS] = self.action_names_or_texts
        return ruth.shared.utils.io.get_dictionary_fingerprint(self_as_dict)
    ...
    def retrieval_intent_templates(self) -> Dict[Text, List[Dict[Text, Any]]]:
        ...
        ruth.shared.utils.io.raise_deprecation_warning(
            ...
            docs=f"{DOCS_URL_MIGRATION_GUIDE}#ruth-23-to-ruth-24",
        )
    ...
    def is_retrieval_intent_template(
        ...
        return ruth.shared.nlu.constants.RESPONSE_IDENTIFIER_DELIMITER in response[0]
    ...
    def _add_requested_slot(self) -> None:
        ...
        """
        if self.form_names and ruth.shared.core.constants.REQUESTED_SLOT not in [
            slot.name for slot in self.slots
        ]:
            self.slots.append(
                TextSlot(
                    ruth.shared.core.constants.REQUESTED_SLOT,
    ...
    def _add_knowledge_base_slots(self) -> None:
        ...
        """
        if (
            ruth.shared.core.constants.DEFAULT_KNOWLEDGE_BASE_ACTION
            ...
            knowledge_base_slots = [
                ruth.shared.core.constants.SLOT_LISTED_ITEMS,
                ruth.shared.core.constants.SLOT_LAST_OBJECT,
                ruth.shared.core.constants.SLOT_LAST_OBJECT_TYPE,
            ]
    ...
    def _add_session_metadata_slot(self) -> None:
        self.slots.append(
            AnySlot(ruth.shared.core.constants.SESSION_START_METADATA_SLOT, )
        )
    ...
    def concatenate_entity_labels(
        ...
        if entity:
            return [
                f"{entity}"
                f"{ruth.shared.core.constants.ENTITY_LABEL_SEPARATOR}"
                f"{sub_label}"
    ...
    def _get_featurized_entities(self, latest_message: UserUttered) -> Set[Text]:
        ...
        intent_name = latest_message.intent.get(
            ruth.shared.nlu.constants.INTENT_NAME_KEY
        )
        ...
        entity_names_roles = set(
            f"{entity['entity']}"
            f"{ruth.shared.core.constants.ENTITY_LABEL_SEPARATOR}{entity['role']}"
            for entity in entities
            if "entity" in entity.keys() and "role" in entity.keys()
        )
        entity_names_groups = set(
            f"{entity['entity']}"
            f"{ruth.shared.core.constants.ENTITY_LABEL_SEPARATOR}{entity['group']}"
            for entity in entities
    ...
    def _get_user_sub_state(
        ...
        entities = tuple(
            self._get_featurized_entities(latest_message).intersection(
                set(sub_state.get(ruth.shared.nlu.constants.ENTITIES, ()))
            )
        )
        ...
        if entities:
            sub_state[ruth.shared.nlu.constants.ENTITIES] = entities
        else:
            sub_state.pop(ruth.shared.nlu.constants.ENTITIES, None)
        ...
        for slot_name, slot in tracker.slots.items():
            if slot is not None and slot.as_feature():
                if omit_unset_slots and not slot.has_been_set:
                    continue
                if slot.value == ruth.shared.core.constants.SHOULD_NOT_BE_SET:
                    slots[slot_name] = ruth.shared.core.constants.SHOULD_NOT_BE_SET
    ...
    def _get_active_loop_sub_state(
        ...
        active_loop: Optional[Text] = tracker.active_loop.get(
            ruth.shared.core.constants.LOOP_NAME
        )
        if active_loop:
            return {ruth.shared.core.constants.LOOP_NAME: active_loop}
        else:
            return {}
    ...
    def get_active_state(
        ...
        """
        state = {
            ruth.shared.core.constants.USER: self._get_user_sub_state(tracker),
            ruth.shared.core.constants.SLOTS: self._get_slots_sub_state(
                tracker, omit_unset_slots=omit_unset_slots
            ),
            ruth.shared.core.constants.PREVIOUS_ACTION: self._get_prev_action_sub_state(
                tracker
            ),
            ruth.shared.core.constants.ACTIVE_LOOP: self._get_active_loop_sub_state(
                tracker
            ),
        }
    ...
    def _remove_rule_only_features(
        ...
        rule_only_slots = rule_only_data.get(
            ruth.shared.core.constants.RULE_ONLY_SLOTS, []
        )
        rule_only_loops = rule_only_data.get(
            ruth.shared.core.constants.RULE_ONLY_LOOPS, []
        )

        # remove slots which only occur in rules but not in stories
        if rule_only_slots:
            for slot in rule_only_slots:
                state.get(ruth.shared.core.constants.SLOTS, {}).pop(slot, None)
        # remove active loop which only occur in rules but not in stories
        if (
            rule_only_loops
            and state.get(ruth.shared.core.constants.ACTIVE_LOOP, {}).get(
                ruth.shared.core.constants.LOOP_NAME
            )
            in rule_only_loops
        ):
            del state[ruth.shared.core.constants.ACTIVE_LOOP]

    @staticmethod
    def _substitute_rule_only_user_input(state: State, last_ml_state: State) -> None:
        if not ruth.shared.core.trackers.is_prev_action_listen_in_state(state):
            if not last_ml_state.get(ruth.shared.core.constants.USER) and state.get(
                ruth.shared.core.constants.USER
            ):
                del state[ruth.shared.core.constants.USER]
            elif last_ml_state.get(ruth.shared.core.constants.USER):
                state[ruth.shared.core.constants.USER] = last_ml_state[
                    ruth.shared.core.constants.USER
                ]
        ...
        for tr, hide_rule_turn in tracker.generate_all_prior_trackers():
            ...
            if ignore_rule_only_turns:
                ...
                if last_ml_action_sub_state:
                    state[
                        ruth.shared.core.constants.PREVIOUS_ACTION
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        ruth.shared.utils.io.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        ruth.shared.utils.io.dump_obj_as_json_to_file(domain_spec_path, metadata)
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        return json.loads(ruth.shared.utils.io.read_file(metadata_path))
    ...
    def _transform_intents_for_file(self) -> List[Union[Text, Dict[Text, Any]]]:
        ...
        for intent_name, intent_props in intent_properties.items():
            if (
                intent_name in ruth.shared.core.constants.DEFAULT_INTENTS
            ...
            use_entities = set(
                ...
                if ruth.shared.core.constants.ENTITY_LABEL_SEPARATOR not in entity
            )
    ...
    def cleaned_domain(self) -> Dict[Text, Any]:
        ...
        for slot in domain_data[KEY_SLOTS].values():
            ...
            if slot["auto_fill"]:
                del slot["auto_fill"]
            if slot["type"].startswith("ruth.shared.core.slots"):
    ...
    def persist(self, filename: Union[Text, Path]) -> None:
        ...
        ruth.shared.utils.io.write_text_file(as_yaml, filename)
    ...
    def as_yaml(self, clean_before_dump: bool = False) -> Text:
        ...
        return ruth.shared.utils.io.dump_obj_as_yaml_to_string(
    ...
    def _actions_for_domain_warnings(self) -> List[Text]:
        ...
        return [
            ...
            if action not in ruth.shared.core.constants.DEFAULT_ACTION_NAMES
        ]
    ...
    def _combine_user_with_default_actions(user_actions: List[Text]) -> List[Text]:
        ...
        return ruth.shared.core.constants.DEFAULT_ACTION_NAMES + unique_user_actions
    ...
    def check_missing_responses(self) -> None:
        ...
        utterances = [
            ...
            if action.startswith(ruth.shared.constants.UTTER_PREFIX)
        ]
    ...
    def is_domain_file(filename: Text) -> bool:
        ...
        """
        from ruth.shared.data import is_likely_yaml_file

        if not is_likely_yaml_file(filename):
            return False

        try:
            content = ruth.shared.utils.io.read_yaml_file(filename)
