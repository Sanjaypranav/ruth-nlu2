...
from ruth.shared.core.slots import TextSlot, ListSlot
from ruth.shared.exceptions import YamlException
import ruth.shared.utils.io
from ruth.shared.core.constants import LOOP_NAME
...
from ruth.shared.core.constants import RULE_SNIPPET_ACTION_NAME
from ruth.shared.core.events import UserUttered, SlotSet, ActiveLoop
from ruth.shared.core.training_data.story_reader.story_reader import StoryReader
...
class YAMLStoryReader(StoryReader):
    ...
    def read_from_file(
        ...
        try:
            return self.read_from_string(
                ruth.shared.utils.io.read_file(
                    filename, ruth.shared.utils.io.DEFAULT_ENCODING
                ),
                skip_validation,
            )
    ...
    def read_from_string(
        ...
        if not skip_validation:
            ruth.shared.utils.validation.validate_yaml_schema(string, CORE_SCHEMA_FILE)

        yaml_content = ruth.shared.utils.io.read_yaml(string)
    ...
    def is_stories_file(cls, file_path: Union[Text, Path]) -> bool:
        ...
        """
        return ruth.shared.data.is_likely_yaml_file(
            file_path
        ) and ruth.shared.utils.io.is_key_in_yaml(file_path, KEY_STORIES, KEY_RULES)
