...
from ruth.shared.core.training_data.story_reader.story_reader import (
    StoryReader,
    StoryParseError,
)
from ruth.shared.core.training_data.structures import StoryStep, FORM_PREFIX
...
class MarkdownStoryReader(StoryReader):
    ...
    def read_from_file(
        ...
        try:
            with open(
                filename, "r", encoding=ruth.shared.utils.io.DEFAULT_ENCODING
            ) as f:
                lines = f.readlines()
    ...
    def parse_e2e_message(
        ...
        if not is_used_for_training and not self.use_e2e:
            ...
            example.data[ruth.shared.nlu.constants.TEXT] = message
            example.data[ruth.shared.nlu.constants.ENTITIES] = []
    ...
    def is_stories_file(file_path: Union[Text, Path]) -> bool:
        ...
        Returns:
            ...
            `False` otherwise.
        """
        if not ruth.shared.data.is_likely_markdown_file(
            file_path
        ) or ruth.shared.data.is_nlu_file(file_path):
            return False

        try:
            with open(
                file_path,
                encoding=ruth.shared.utils.io.DEFAULT_ENCODING,
    ...
    def is_test_stories_file(file_path: Union[Text, Path]) -> bool:
        ...
        """
        if not ruth.shared.data.is_likely_markdown_file(file_path):
            return False

        dirname = os.path.dirname(file_path)
        return (
            "tests" in dirname
            and ruth.shared.data.is_story_file(file_path)
            and not ruth.shared.data.is_nlu_file(file_path)
        )
