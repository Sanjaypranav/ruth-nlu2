...
from ruth import model, telemetry
from ruth.model import FingerprintComparisonResult
...
import ruth.shared.utils.common
from ruth.utils.common import TempDirectoryPath

from ruth.shared.utils.cli import (
    print_success,
    print_warning,
)
import ruth.shared.exceptions
...
async def _train_async_internal(
    ...
    if nlu_data.has_e2e_examples():
        ruth.shared.utils.common.mark_as_experimental_feature("end-to-end training")
...
async def _do_training(
    ...
    if fingerprint_comparison_result.should_retrain_nlu():
        ...
        ruth.shared.utils.cli.print_color(
            "NLU data/configuration did not change. No need to retrain NLU model.",
            color=ruth.shared.utils.io.bcolors.OKBLUE,
        )
...
def _load_interpreter(
    ...
    if interpreter_path:
        return ruth.core.interpreter.create_interpreter(interpreter_path)
...
def _interpreter_from_previous_model(
    ...
    with model.unpack_model(old_model_zip_path) as unpacked:
        ...
        return ruth.core.interpreter.create_interpreter(old_nlu)
...
def train_core(
    ...
    return ruth.utils.common.run_in_loop(
        train_core_async(
            domain=domain,
            config=config,
            stories=stories,
...
async def train_core_async(
    ...
    if nlu_data.has_e2e_examples():
        ruth.shared.utils.cli.print_error(
            "Stories file contains e2e stories. Please train using `ruth train` so that"
...
async def _train_core_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        ruth.shared.utils.cli.print_color(
            "Training Core model...", color=ruth.shared.utils.io.bcolors.OKBLUE
        )
        ...
        ruth.shared.utils.cli.print_color(
            "Core model training completed.", color=ruth.shared.utils.io.bcolors.OKBLUE
        )
...
async def _train_nlu_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        config = await file_importer.get_config()
        ruth.shared.utils.cli.print_color(
            "Training NLU model...", color=ruth.shared.utils.io.bcolors.OKBLUE
        )
        ...
        ruth.shared.utils.cli.print_color(
            "NLU model training completed.", color=ruth.shared.utils.io.bcolors.OKBLUE
        )
