...
from ruth.shared.nlu.training_data.formats import RasaYAMLReader
from ruth import model
from ruth.constants import DEFAULT_RESPONSE_TIMEOUT, MINIMUM_COMPATIBLE_VERSION
...
from ruth.core.channels.channel import (
    CollectingOutputChannel,
    OutputChannel,
    UserMessage,
)
import ruth.shared.core.events
from ruth.shared.core.events import Event
from ruth.core.lock_store import LockStore
from ruth.core.test import test
from ruth.core.tracker_store import TrackerStore
from ruth.shared.core.trackers import DialogueStateTracker, EventVerbosity
from ruth.core.utils import AvailableEndpoints
from ruth.nlu.emulators.no_emulator import NoEmulator
import ruth.nlu.test
from ruth.nlu.test import CVEvaluationResult
from ruth.shared.utils.schemas.events import EVENTS_SCHEMA
from ruth.utils.endpoints import EndpointConfig

if TYPE_CHECKING:
    from ssl import SSLContext  # noqa: F401
    from ruth.core.processor import MessageProcessor
...
def requires_auth(
    ...
    def decorator(f: "SanicView") -> "SanicView":
        def conversation_id_from_args(args: Any, kwargs: Any) -> Optional[Text]:
            argnames = ruth.shared.utils.common.arguments_of(f)
        ...
        async def sufficient_scope(
            ...
            jwt_data = await ruth.utils.common.call_potential_coroutine(
        ...
        async def decorated(
            ...
            if token is not None and provided == token:
                ...
                "USE_JWT"
            ) and await ruth.utils.common.call_potential_coroutine(
                # This is a coroutine since `sanic-jwt==1.6`
...
async def update_conversation_with_events(
    ...
    """
    if ruth.shared.core.events.do_events_begin_with_session_start(events):
...
def _create_emulator(mode: Optional[Text]) -> NoEmulator:
    ...
    if mode is None:
        ...
        from ruth.nlu.emulators.wit import WitEmulator

        return WitEmulator()
    elif mode.lower() == "luis":
        from ruth.nlu.emulators.luis import LUISEmulator

        return LUISEmulator()
    elif mode.lower() == "dialogflow":
        from ruth.nlu.emulators.dialogflow import DialogflowEmulator
...
async def _load_agent(
    ...
    try:
        ...
        loaded_agent = await ruth.core.agent.load_agent(
            model_path,
            model_server,
            remote_storage,
            generator=generator,
...
def add_root_route(app: Sanic) -> None:
    ...
    async def hello(request: Request) -> HTTPResponse:
        ...
        return response.text("Hello from Rasa: " + ruth.__version__)
...
def create_app(
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        until_time = ruth.utils.endpoints.float_arg(request, "until")
    ...
    async def append_events(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        try:
            async with app.agent.lock_store.lock(conversation_id):
                ...
                if ruth.utils.endpoints.bool_arg(
    ...
    async def retrieve_story(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        until_time = ruth.utils.endpoints.float_arg(request, "until")
        fetch_all_sessions = ruth.utils.endpoints.bool_arg(
    ...
    async def train(request: Request, temporary_directory: Path) -> HTTPResponse:
        ...
        try:
            ...
            from ruth.model_training import train_async
    ...
    async def evaluate_stories(
        ...
        use_e2e = ruth.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def _evaluate_model_using_test_set(
        ...
        return await ruth.nlu.test.run_evaluation(
    ...
    async def _cross_validate(data_file: Text, config_file: Text, folds: int) -> Dict:
        ...
        evaluations = ruth.nlu.test.cross_validate(
            data=nlu_data,
            n_folds=folds,
            nlu_config=config,
...
def _training_payload_from_json(request: Request, temp_dir: Path) -> Dict[Text, Any]:
    ...
    ruth.shared.utils.io.write_text_file(request_payload["config"], config_path)

    if "nlu" in request_payload:
        nlu_path = os.path.join(temp_dir, "nlu.md")
        ruth.shared.utils.io.write_text_file(request_payload["nlu"], nlu_path)

    if "stories" in request_payload:
        stories_path = os.path.join(temp_dir, "stories.md")
        ruth.shared.utils.io.write_text_file(request_payload["stories"], stories_path)

    if "responses" in request_payload:
        responses_path = os.path.join(temp_dir, "responses.md")
        ruth.shared.utils.io.write_text_file(
    ...
    if "domain" in request_payload:
        ...
        ruth.shared.utils.io.write_text_file(request_payload["domain"], domain_path)

    model_output_directory = str(temp_dir)
    if request_payload.get(
        "save_to_default_model_directory",
        ruth.utils.endpoints.bool_arg(request, "save_to_default_model_directory", True),
    ):
    ...
    return dict(
        ...
        force_training=request_payload.get(
            "force", ruth.utils.endpoints.bool_arg(request, "force_training", False)
        ),
...
def _training_payload_from_yaml(
    ...
    decoded = request.body.decode(ruth.shared.utils.io.DEFAULT_ENCODING)
    _validate_yaml_training_payload(decoded)

    training_data = temp_dir / file_name
    ruth.shared.utils.io.write_text_file(decoded, training_data)

    model_output_directory = str(temp_dir)
    if ruth.utils.endpoints.bool_arg(request, "save_to_default_model_directory", True):
        model_output_directory = DEFAULT_MODELS_PATH
    ...
    return dict(
        ...
        training_files=str(temp_dir),
        output=model_output_directory,
        force_training=ruth.utils.endpoints.bool_arg(request, "force_training", False),
        core_additional_arguments=_extract_core_additional_arguments(request),
        nlu_additional_arguments=_extract_nlu_additional_arguments(request),
    )
...
def _extract_core_additional_arguments(request: Request) -> Dict[Text, Any]:
    return {
        "augmentation_factor": ruth.utils.endpoints.int_arg(
...
def _extract_nlu_additional_arguments(request: Request) -> Dict[Text, Any]:
    return {
        "num_threads": ruth.utils.endpoints.int_arg(request, "num_threads", 1),
    }
