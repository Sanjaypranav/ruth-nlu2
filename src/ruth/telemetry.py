...
# e.g. `RASA_TELEMETRY_WRITE_KEY=12354 ruth train`
...
def print_telemetry_reporting_info() -> None:
    ...
    message = textwrap.dedent(
        ...
        If you'd like to opt-out, you can use `ruth telemetry disable`.
...
def _fetch_write_key(tool: Text, environment_variable: Text) -> Optional[Text]:
    ...
    from ruth import __name__ as name
...
def _is_docker() -> bool:
    ...
    try:
        return "docker" in ruth.shared.utils.io.read_file("/proc/self/cgroup", "utf8")
    except Exception:  # skipcq:PYL-W0703
        return False
...
def _default_context_fields() -> Dict[Text, Any]:
    ...
    if not TELEMETRY_CONTEXT:
        ...
        TELEMETRY_CONTEXT = {
            ...
            "rasa_open_source": ruth.__version__,
...
def initialize_error_reporting() -> None:
    ...
    # for local testing, set the key using `RASA_EXCEPTION_WRITE_KEY=key ruth <command>`
    ...
    sentry_sdk.init(
        ...
        in_app_include=["ruth"],  # only submit errors in this package
        with_locals=False,  # don't submit local variables
        release=f"ruth-{ruth.__version__}",
...
async def track_model_training(
    ...
    Args:
        ...
        model_type: Specifies the type of training, should be either "ruth", "core"
            or "nlu".
...
def track_server_start(
    ...
    """Track when a user starts a ruth server.
    ...
    Args:
        ...
        is_api_enabled: whether the ruth API server is enabled
    """
    from ruth.core.utils import AvailableEndpoints
...
def track_project_init(path: Text) -> None:
    """Track when a user creates a project using ruth init.
...
def track_shell_started(model_type: Text) -> None:
    """Track when a user starts a bot using ruth shell.

    Args:
        model_type: Type of the model, core / nlu or ruth."""
