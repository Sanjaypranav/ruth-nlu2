...
import aiohttp
from ruth.exceptions import MissingDependencyException
import ruamel.yaml as yaml

from ruth import telemetry
from ruth.cli import SubParsersAction
from ruth.cli.arguments import x as arguments
...
from ruth.shared.exceptions import RasaXTermsError
...
def _rasa_service(
    ...
    from ruth.core.run import serve_application

    # needs separate logging configuration as it is started in its own process
    ruth.utils.common.set_log_level(args.loglevel)
    ruth.utils.io.configure_colored_logging(args.loglevel)
...
def _prepare_credentials_for_rasa_x(
    ...
    credentials_path = ruth.cli.utils.get_validated_path(
        credentials_path, "credentials", DEFAULT_CREDENTIALS_PATH, True
    )
    if credentials_path:
        credentials = ruth.shared.utils.io.read_config_file(credentials_path)
    else:
        credentials = {}

    # this makes sure the Rasa X is properly configured no matter what
    if rasa_x_url:
        credentials["ruth"] = {"url": rasa_x_url}
    dumped_credentials = yaml.dump(credentials, default_flow_style=False)
    tmp_credentials = ruth.utils.io.create_temporary_file(dumped_credentials, "yml")
...
def generate_rasa_x_token(length: int = 16) -> Text:
    ...
    A new token is generated on every `ruth x` command.
    """
...
def _configure_logging(args: argparse.Namespace) -> None:
    from ruth.core.utils import configure_file_logging
    from ruth.utils.common import set_log_level
...
def _validate_rasa_x_start(args: argparse.Namespace, project_path: Text) -> None:
    if not is_rasa_x_installed():
        ruth.shared.utils.cli.print_error_and_exit(
            "Rasa X is not installed. The `ruth x` "
    ...
    if args.port == args.rasa_x_port:
        ruth.shared.utils.cli.print_error_and_exit(
            ...
            "different ports using the arguments '--port' and '--ruth-x-port'.".format(
    ...
    if not is_rasa_project_setup(args, project_path):
        ruth.shared.utils.cli.print_error_and_exit(
            "This directory is not a valid Rasa project. Use 'ruth init' "
...
def rasa_x(args: argparse.Namespace) -> None:
    from ruth.cli.utils import signal_handler
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(config_endpoint) as resp:
                    if resp.status == 200:
                        ...
                        try:
                            return [
                                ruth.utils.io.create_temporary_file(rjs[k])
                                for k in keys
                            ]
...
def run_in_production(args: argparse.Namespace) -> None:
    from ruth.shared.utils.cli import print_success
...
def _get_config_path(args: argparse.Namespace,) -> Optional[Text]:
    config_path = ruth.cli.utils.get_validated_path(
...
def _get_domain_path(args: argparse.Namespace,) -> Optional[Text]:
    domain_path = ruth.cli.utils.get_validated_path(
...
def _get_credentials_and_endpoints_paths(
    ...
    if config_endpoint:
        endpoints_config_path, credentials_path = ruth.utils.common.run_in_loop(
            _pull_runtime_config_from_server(config_endpoint)
        )

    else:
        endpoints_config_path = ruth.cli.utils.get_validated_path(
