...
from ruth.cli.arguments import export as arguments
from ruth.shared.constants import DOCS_URL_EVENT_BROKERS, DOCS_URL_TRACKER_STORES
from ruth.exceptions import PublishingError
from ruth.shared.exceptions import RasaException
from ruth.core.brokers.pika import PikaEventBroker
...
def export_trackers(args: argparse.Namespace) -> None:
    ...
    Args:
        args: Command-line arguments to process.
    """
    ruth.utils.common.run_in_loop(_export_trackers(args))
...
async def _export_trackers(args: argparse.Namespace) -> None:
    ...
    endpoints = ruth.core.utils.read_endpoints_from_path(args.endpoints)
    ...
    try:
        ...
        ruth.shared.utils.cli.print_error_and_exit(str(e))


def _get_continuation_command(exporter: "Exporter", timestamp: float) -> Text:
    """Build CLI command to continue 'ruth export' where it was interrupted.
