...
from ruth.cli.arguments import test as arguments
...
def add_subparser(
    ...
    test_parser = subparsers.add_parser(
        ...
        help="Tests ruth models using your test NLU data and stories.",
    )
    ...
    test_core_parser = test_subparsers.add_parser(
        ...
        conflict_handler="resolve",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Tests ruth Core models using your test stories.",
    )
    ...
    test_nlu_parser = test_subparsers.add_parser(
        ...
        parents=parents,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Tests ruth NLU models using your test NLU data.",
    )
...
def run_core_test(args: argparse.Namespace) -> None:
    """Run core tests."""
    from ruth.model_testing import (
        test_core_models_in_directory,
        test_core,
        test_core_models,
    )

    stories = ruth.cli.utils.get_validated_path(
...
async def run_nlu_test_async(
    ...
    Args:
        ...
        models_path: path to a trained ruth model.
    ...
    data_path = ruth.cli.utils.get_validated_path(data_path, "nlu", DEFAULT_DATA_PATH)
    ...
    if config is not None and len(config) == 1:
        ...
        if os.path.isdir(config):
            config = ruth.shared.utils.io.list_files(config)
        ...
        for file in config:
            try:
                validation_utils.validate_yaml_schema(
                    ruth.shared.utils.io.read_file(file), CONFIG_SCHEMA_FILE,
                )
...
def run_nlu_test(args: argparse.Namespace) -> None:
    ...
    Args:
        args: the parsed CLI arguments for 'ruth test nlu'.
    """
