...
from ruth.cli.shell import shell
from ruth.cli.utils import create_output_path
from ruth.shared.utils.cli import print_success, print_error_and_exit
...
def print_train_or_instructions(args: argparse.Namespace, path: Text) -> None:
    ...
    if should_train:
        ...
        training_result = ruth.train(domain, config, training_files, output)
        ...
        print_success(
            ...
            "to the project directory and running 'ruth train'."
        )
    ...
    if should_run:
        ...
        if args.no_prompt:
            print(
                ...
                "run 'ruth shell' at any time inside "
...
def print_cancel() -> None:
    print_success("Ok. You can continue setting up by running 'ruth init' 🙋🏽‍♀️")
    sys.exit(0)
