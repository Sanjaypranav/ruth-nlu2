...
import ruth.cli.arguments.train as train_arguments

import ruth.cli.utils
import ruth.utils.common
from ruth.core.train import do_compare_training
...
def add_subparser(
    ...
    train_parser = subparsers.add_parser(
        "train",
        help="Trains a ruth model using your NLU data and stories.",
        parents=parents,
    ...
    train_core_parser = train_subparsers.add_parser(
        ...
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Trains a ruth Core model using your stories.",
    )
    ...
    train_nlu_parser = train_subparsers.add_parser(
        ...
        parents=parents,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Trains a ruth NLU model using your NLU data.",
    )
...
def run_training(args: argparse.Namespace, can_exit: bool = False) -> Optional[Text]:
    ...
    """
    from ruth import train as train_all

    domain = ruth.cli.utils.get_validated_path(
    ...
    training_files = [
        ruth.cli.utils.get_validated_path(
...
def run_core_training(
    ...
    Returns:
        Path to a trained model or `None` if training was not successful.
    """
    from ruth.model_training import train_core

    output = train_path or args.out

    args.domain = ruth.cli.utils.get_validated_path(
        args.domain, "domain", DEFAULT_DOMAIN_PATH, none_is_valid=True
    )
    story_file = ruth.cli.utils.get_validated_path(
...
def run_nlu_training(
    ...
    Returns:
        Path to a trained model or `None` if training was not successful.
    """
    from ruth.model_training import train_nlu

    output = train_path or args.out

    config = _get_valid_config(args.config, CONFIG_MANDATORY_KEYS_NLU)
    nlu_data = ruth.cli.utils.get_validated_path(
...
def _get_valid_config(
    ...
    """
    config = ruth.cli.utils.get_validated_path(config, "config", default_config)
    ...
    missing_keys = ruth.cli.utils.missing_config_keys(config, mandatory_keys)
    if missing_keys:
        print_error(
