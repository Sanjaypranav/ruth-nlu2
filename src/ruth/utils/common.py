...
from ruth.constants import DEFAULT_LOG_LEVEL_LIBRARIES, ENV_LOG_LEVEL_LIBRARIES
from ruth.shared.constants import DEFAULT_LOG_LEVEL, ENV_LOG_LEVEL
...
def read_global_config(path: Text) -> Dict[Text, Any]:
    ...
    try:
        return ruth.shared.utils.io.read_config_file(path)
    except Exception:
...
def set_log_level(log_level: Optional[int] = None) -> None:
    ...
    logging.getLogger("ruth").setLevel(log_level)
...
def write_global_config_value(name: Text, value: Any) -> bool:
    ...
    """
    # need to use `ruth.constants.GLOBAL_USER_CONFIG_PATH` to allow patching
    # in tests
    config_path = ruth.constants.GLOBAL_USER_CONFIG_PATH
    try:
        os.makedirs(os.path.dirname(config_path), exist_ok=True)

        c = read_global_config(config_path)
        c[name] = value
        ruth.shared.utils.io.write_yaml(c, ruth.constants.GLOBAL_USER_CONFIG_PATH)
        return True
