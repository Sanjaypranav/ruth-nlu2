...
from ruth.shared.nlu.constants import TEXT, INTENT_NAME_KEY
from ruth.shared.nlu.training_data.loading import MARKDOWN, RASA, RASA_YAML
...
from ruth.core import run, utils
import ruth.core.train
from ruth.core.constants import DEFAULT_SERVER_FORMAT, DEFAULT_SERVER_PORT
...
from ruth.shared.core.trackers import EventVerbosity, DialogueStateTracker
from ruth.shared.core.training_data import visualization
from ruth.shared.core.training_data.visualization import (
    VISUALIZATION_TEMPLATE_PATH,
    visualize_neighborhood,
)
from ruth.core.utils import AvailableEndpoints
from ruth.shared.importers.rasa import TrainingDataImporter
from ruth.utils.common import update_sanic_log_level
from ruth.utils.endpoints import EndpointConfig
from ruth.shared.exceptions import InvalidConfigException
...
def format_bot_output(message: BotUttered) -> Text:
    ...
    if data.get("buttons"):
        output += "\nButtons:"
        choices = ruth.cli.utils.button_choices_from_message_data(
    ...
    if data.get("elements"):
        ...
        for idx, element in enumerate(data.get("elements")):
            element_str = ruth.cli.utils.element_to_string(element, idx)
            output += "\n" + element_str
...
def _slot_history(tracker_dump: Dict[Text, Any]) -> List[Text]:
    ...
    for k, s in tracker_dump.get("slots", {}).items():
        colored_value = ruth.shared.utils.io.wrap_with_color(
            str(s), color=ruth.shared.utils.io.bcolors.WARNING
        )
    ...
    questions = questionary.form(
        export_stories=questionary.text(
            ...
            validate=io_utils.file_type_validator(
                ruth.shared.data.MARKDOWN_FILE_EXTENSIONS
                + ruth.shared.data.YAML_FILE_EXTENSIONS,
        ...
        export_nlu=questionary.text(
            ...
            default=PATHS["nlu"],
            validate=io_utils.file_type_validator(
                list(ruth.shared.data.TRAINING_DATA_EXTENSIONS),
...
def _split_conversation_at_restarts(
    ...
    split_events = ruth.shared.core.events.split_events(
...
def _collect_messages(events: List[Dict[Text, Any]]) -> List[Message]:
    ...
    import ruth.shared.nlu.training_data.util as rasa_nlu_training_data_utils
...
def _write_stories_to_file(
    ...
    from ruth.shared.core.training_data.story_writer.markdown_story_writer import (
        MarkdownStoryWriter,
    )
    ...
    if ruth.shared.data.is_likely_yaml_file(export_story_path):
    ...
    with open(
        export_story_path, append_write, encoding=ruth.shared.utils.io.DEFAULT_ENCODING
    ) as f:
        interactive_story_counter = 1
        for conversation in sub_conversations:
            parsed_events = ruth.shared.core.events.deserialise_events(conversation)
...
def _write_nlu_to_file(export_nlu_path: Text, events: List[Dict[Text, Any]]) -> None:
    ...
    ruth.shared.utils.io.write_text_file(stringified_training_data, export_nlu_path)
...
def _get_nlu_target_format(export_path: Text) -> Text:
    ...
    if guessed_format not in {MARKDOWN, RASA, RASA_YAML}:
        if ruth.shared.data.is_likely_json_file(export_path):
            guessed_format = RASA
        elif ruth.shared.data.is_likely_markdown_file(export_path):
            guessed_format = MARKDOWN
        elif ruth.shared.data.is_likely_yaml_file(export_path):
    ...
    collected_actions = list(
        {
            e["name"]
            for e in actions
            if e["name"] not in ruth.shared.core.constants.DEFAULT_ACTION_NAMES
...
async def _predict_till_next_listen(
    ...
    if len(events) >= 2:
        ...
        if last_event.get("event") == BotUttered.type_name and last_event["data"].get(
            ...
            if user_selection != ruth.cli.utils.FREE_TEXT_INPUT_PROMPT:
...
def _get_button_choice(last_event: Dict[Text, Any]) -> Text:
    ...
    return ruth.cli.utils.payload_from_button_question(question)
...
async def _confirm_form_validation(
    ...
    if not validate_input:
        ...
        await send_event(
            ...
            conversation_id,
            {
                "event": ruth.shared.core.events.LoopInterrupted.type_name,
...
def _as_md_message(parse_data: Dict[Text, Any]) -> Text:
    ...
    from ruth.shared.nlu.training_data.formats.readerwriter import TrainingDataWriter
...
async def _correct_entities(
    ...
    from ruth.shared.nlu.training_data import entities_parser
...
def _print_help(skip_visualization: bool) -> None:
    ...
    ruth.shared.utils.cli.print_success(
...
async def _get_training_trackers(
    ...
    from ruth.core import training
