...
from ruth.core import constants
...
def create_interpreter(
    obj: Union[
        ruth.shared.nlu.interpreter.NaturalLanguageInterpreter,
        EndpointConfig,
        Text,
        None,
    ]
    ...
    if isinstance(obj, ruth.shared.nlu.interpreter.NaturalLanguageInterpreter):
        return obj
        ...
        return ruth.shared.nlu.interpreter.RegexInterpreter()
    else:
        return _create_from_endpoint_config(obj)


class RasaNLUHttpInterpreter(ruth.shared.nlu.interpreter.NaturalLanguageInterpreter):
    ...
    async def _rasa_http_parse(
        ...
        """Send a text message to a running ruth NLU http server.
        Return `None` on failure."""

        if not self.endpoint_config:
            logger.error(
                f"Failed to parse text '{text}' using ruth NLU over http. "
                f"No ruth NLU server specified!"
            )
            return None
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=params) as resp:
                    if resp.status == 200:
                        ...
                        logger.error(
                            f"Failed to parse text '{text}' using ruth NLU over "
            ...
            logger.exception(f"Failed to parse text '{text}' using ruth NLU over http.")
            return None


class RasaNLUInterpreter(ruth.shared.nlu.interpreter.NaturalLanguageInterpreter):
    def __init__(
        self,
...
def _load_from_module_name_in_endpoint_config(
    ...
    try:
        nlu_interpreter_class = ruth.shared.utils.common.class_from_module_path(
