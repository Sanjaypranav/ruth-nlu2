...
from ruth.shared.core.events import Event, ActionExecuted, UserUttered
...
from ruth.shared.nlu.constants import TEXT, INTENT, ENTITIES
from ruth.utils.tensorflow.constants import LABEL_PAD_ID
...
from ruth.core.exceptions import InvalidTrackerFeaturizerUsageError
from ruth.shared.nlu.constants import ACTION_NAME
...
class TrackerFeaturizer:
    ...
    def _choose_last_user_input(
        ...
        for states in trackers_as_states:
            ...
            if not ruth.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                continue
    ...
    def persist(self, path: Union[Text, Path]) -> None:
        ...
        ruth.shared.utils.io.create_directory_for_file(featurizer_file)
    ...
    def load(path: Text) -> Optional["TrackerFeaturizer"]:
        ...
        if featurizer_file.is_file():
            return jsonpickle.decode(ruth.shared.utils.io.read_file(featurizer_file))
...
class FullDialogueTrackerFeaturizer(TrackerFeaturizer):
    ...
    def training_states_labels_and_entities(
        ...
        pbar = tqdm(
            ...
            disable=ruth.shared.utils.io.is_logging_disabled(),
        )
        for tracker in pbar:
    ...
    def _cleanup_last_user_state_with_action_listen(
        ...
        for states in trackers_as_states:
            if not states:
                continue
            last_state = states[-1]
            if ruth.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                del states[-1]
