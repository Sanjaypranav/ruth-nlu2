...
import ruth.utils.train_utils
import tensorflow as tf
import tensorflow_addons as tfa
from typing import Any, List, Optional, Text, Dict, Tuple, Union, TYPE_CHECKING, Type

import ruth.utils.io as io_utils
import ruth.core.actions.action
from ruth.nlu.constants import TOKENS_NAMES
from ruth.nlu.extractors.extractor import EntityExtractor, EntityTagSpec
from ruth.shared.core.domain import Domain
from ruth.core.featurizers.tracker_featurizers import (
    TrackerFeaturizer,
...
from ruth.shared.nlu.constants import (
    ACTION_TEXT,
    ACTION_NAME,
    INTENT,
    TEXT,
...
from ruth.core.constants import DEFAULT_POLICY_PRIORITY, DIALOGUE
from ruth.shared.constants import DIAGNOSTIC_DATA
from ruth.shared.core.constants import ACTIVE_LOOP, SLOTS, ACTION_LISTEN_NAME
from ruth.shared.core.trackers import DialogueStateTracker
from ruth.shared.core.generator import TrackerWithCachedStates
import ruth.utils.train_utils
from ruth.utils.tensorflow.models import RasaModel, TransformerRasaModel
from ruth.utils.tensorflow import rasa_layers
from ruth.utils.tensorflow.model_data import (
    RasaModelData,
    FeatureSignature,
    FeatureArray,
    Data,
)
from ruth.utils.tensorflow.model_data_utils import convert_to_data_format
from ruth.utils.tensorflow.constants import (
    LABEL,
    IDS,
    TRANSFORMER_SIZE,
...
from ruth.shared.core.events import EntitiesAdded, Event
from ruth.shared.nlu.training_data.message import Message
from ruth.shared.utils import io as shared_io_utils
...
class TEDPolicy(Policy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.split_entities_config = ruth.utils.train_utils.init_split_entities(
        ...
        if self.config[CHECKPOINT_MODEL]:
            self.tmp_checkpoint_dir = Path(ruth.utils.io.create_temporary_directory())
    ...
    def _load_params(self, **kwargs: Dict[Text, Any]) -> None:
        new_config = ruth.utils.train_utils.check_core_deprecated_options(kwargs)
        self.config = ruth.utils.train_utils.override_defaults(
    ...
    def _auto_update_configuration(self) -> None:
        ...
        self.config = ruth.utils.train_utils.update_confidence_type(self.config)
        ruth.utils.train_utils.validate_configuration_settings(self.config)
        self.config = ruth.utils.train_utils.update_deprecated_loss_type(self.config)
        self.config = ruth.utils.train_utils.update_similarity_type(self.config)
        self.config = ruth.utils.train_utils.update_evaluation_parameters(self.config)
        self.config = ruth.utils.train_utils.update_deprecated_sparsity_to_density(
            self.config
        )
    ...
    def run_training(
        ...
        ) = ruth.utils.train_utils.create_data_generators(
            model_data,
        ...
        callbacks = ruth.utils.train_utils.create_common_callbacks(
            self.config[EPOCHS],
    ...
    def predict_action_probabilities(
        ...
        **kwargs: Any,
        ...
        Args:
            tracker: the :class:`ruth.core.trackers.DialogueStateTracker`
            domain: the :class:`ruth.shared.core.domain.Domain`
        ...
        if self.config[RANKING_LENGTH] > 0 and self.config[MODEL_CONFIDENCE] == SOFTMAX:
            ...
            confidence = ruth.utils.train_utils.normalize(
    ...
    def _create_optional_event_for_entities(
        ...
        predicted_tags, confidence_values = ruth.utils.train_utils.entity_label_to_tags(
            prediction_output,
            self._entity_tag_specs,
    ...
    def persist(self, path: Union[Text, Path]) -> None:
        ...
        ruth.shared.utils.io.create_directory_for_file(tf_model_file)
    ...
    def persist_model_utilities(self, model_path: Path) -> None:
        ...
        ruth.shared.utils.io.dump_obj_as_json_to_file(
    ...
    def _load_model_utilities(cls, model_path: Path) -> Dict[Text, Any]:
        ...
        entity_tag_specs = ruth.shared.utils.io.read_json_file(
    ...
    def _update_loaded_params(cls, meta: Dict[Text, Any]) -> Dict[Text, Any]:
        meta = ruth.utils.train_utils.override_defaults(cls.defaults, meta)
        meta = ruth.utils.train_utils.update_confidence_type(meta)
        meta = ruth.utils.train_utils.update_similarity_type(meta)
        meta = ruth.utils.train_utils.update_deprecated_loss_type(meta)
