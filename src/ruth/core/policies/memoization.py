...
from ruth.shared.constants import DOCS_URL_POLICIES
...
from ruth.shared.utils.io import is_logging_disabled
from ruth.core.constants import MEMOIZATION_POLICY_PRIORITY
    ...
    def _create_feature_key(self, states: List[State]) -> Text:
        ...
        if self.ENABLE_FEATURE_STRING_COMPRESSION:
            compressed = zlib.compress(
                bytes(feature_str, ruth.shared.utils.io.DEFAULT_ENCODING)
            )
            return base64.b64encode(compressed).decode(
                ruth.shared.utils.io.DEFAULT_ENCODING
            )
        else:
            return feature_str
