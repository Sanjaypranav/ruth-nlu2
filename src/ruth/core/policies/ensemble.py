...
import ruth.core
import ruth.core.training.training
from ruth.core.constants import FALLBACK_POLICY_PRIORITY
from ruth.shared.exceptions import RasaException, InvalidConfigException
import ruth.shared.utils.common
import ruth.shared.utils.io
import ruth.utils.io
from ruth.constants import MINIMUM_COMPATIBLE_VERSION
...
from ruth.core.exceptions import UnsupportedDialogueModelError
from ruth.core.featurizers.tracker_featurizers import MaxHistoryTrackerFeaturizer
from ruth.shared.nlu.interpreter import NaturalLanguageInterpreter, RegexInterpreter
from ruth.core.policies.policy import Policy, SupportedData, PolicyPrediction
from ruth.core.policies.fallback import FallbackPolicy
from ruth.core.policies.memoization import MemoizationPolicy, AugmentedMemoizationPolicy
from ruth.core.policies.rule_policy import RulePolicy
from ruth.core.training import training
from ruth.shared.core.trackers import DialogueStateTracker
from ruth.shared.core.generator import TrackerWithCachedStates
from ruth.core import registry
...
class PolicyEnsemble:
    versioned_packages = ["ruth", "tensorflow", "sklearn"]
    ...
    def _persist_metadata(self, path: Text) -> None:
        ...
        policy_names = [
            ruth.shared.utils.common.module_path_from_instance(p) for p in self.policies
        ]
    ...
    def load_metadata(cls, path: Text) -> Dict[Text, Any]:
        ...
        metadata = json.loads(
            ruth.shared.utils.io.read_file(os.path.abspath(metadata_path))
        )
        return metadata
    ...
    def ensure_model_compatibility(
        ...
        model_version = metadata.get("ruth", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedDialogueModelError(
                ...
                "".format(model_version, ruth.__version__, version_to_check),
                model_version,
            )
    ...
    def load(
        ...
        for i, policy_name in enumerate(metadata["policy_names"]):
            ...
            if "kwargs" not in ruth.shared.utils.common.arguments_of(policy_cls.load):
                if context:
                    raise UnsupportedDialogueModelError(
        ...
        ensemble_cls = ruth.shared.utils.common.class_from_module_path(
...
class SimplePolicyEnsemble(PolicyEnsemble):
    ...
    def _get_prediction(
        ...
        arguments = ruth.shared.utils.common.arguments_of(
